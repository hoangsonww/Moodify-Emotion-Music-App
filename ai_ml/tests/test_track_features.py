...
class TestEmotionBucket:
    ...
    def test_case_insensitive(self):
        v = tf.featurize({}, context_emotion="  JoY  ")
        assert v[tf.EMOTIONS.index("joy")] == 1.0
...
class TestDurationBucket:
    ...
    def test_garbage_duration_is_median_bucket(self):
        v = tf.featurize({"duration_ms": "huh?"}, context_emotion="neutral")
        offset = len(tf.EMOTIONS) + len(tf.DECADES)
        assert v[offset + tf.DURATION_BUCKETS.index("2_4m")] == 1.0
...
class TestBatchFeaturize:
    ...
    def test_ignores_garbage_popularity(self):

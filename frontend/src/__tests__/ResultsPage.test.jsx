...
describe('<ResultsPage />', () => {
  ...
  const setup = () => {
    render(
      <DarkModeContext.Provider value={{ isDarkMode: false }}>
        <MemoryRouter initialEntries={[{ pathname: '/results', state: initialState }]}>
          <Routes>
  ...
  it('renders detected mood and initial recommendation cards', () => {
    ...
    expect(
      screen.getByText('Happy', { selector: 'span' })
    ).toBeInTheDocument()
  ...
  it('updates recommendations when mood is changed', async () => {
    const newRecs = [
      { name: 'Song C', artist: 'Artist C', image_url: '', preview_url: null, external_url: '' },
    ]
  ...
  it('updates recommendations when region is changed', async () => {
    const newRecs = [
      { name: 'Song D', artist: 'Artist D', image_url: '', preview_url: null, external_url: '' },
    ]
    axios.post.mockResolvedValueOnce({ data: { recommendations: newRecs } })

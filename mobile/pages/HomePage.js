...
const HomePage = () => {
  ...
  const saveToHistory = async (mood, recommendations) => {
    try {
      ...
      // Save mood history
      if (mood) {
        await axios.post(`${API_URL}/users/mood_history/${userData.id}/`, { mood }, {
          headers: {
            'Authorization': `Bearer ${token}`,
      ...
      if (recommendations && recommendations.length > 0) {
        await axios.post(`${API_URL}/users/recommendations/${userData.id}/`, {
      ...
      const updatedUserData = await axios.get(`${API_URL}/users/user/profile/`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (activeTab === 'text') {
        ...
        response = await axios.post(`${MODAL_API_URL}/facial_emotion`, formData, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
      } else if (activeTab === 'speech') {
        response = await axios.post(`${MODAL_API_URL}/speech_emotion`, formData, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
  ...
  const confirmImage = async () => {
    try {
      ...
      const response = await axios.post(`${MODAL_API_URL}/facial_emotion`, formData, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleAudioUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${MODAL_API_URL}/speech_emotion`, formData, {
        headers: {
